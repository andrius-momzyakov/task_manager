...
from vctasks.addtask.forms import SetFilterForm, ExecuteTaskForm, AppointTaskForm,AddTaskForm, AddFileForm
...
def common_tasklist(request, page_number=None, ):
    ...
    if status=='new':
        qs = get_filtered_tasklist(request.user).filter(responsible__isnull=True)
        status_name = u' - НОВЫЕ'
    elif status=='closed':
        ...
        qs = get_filtered_tasklist(request.user).\
            ...
            ready_date__isnull=True, start_date__isnull=False)
        status_name = u' - В ОБРАБОТКЕ'
    elif status=='set':
        qs = get_filtered_tasklist(request.user).\
            filter(responsible__isnull=False, date_close__isnull=True, \
            ready_date__isnull=True, start_date__isnull=True, appoint_date__isnull=False)
        status_name = u' - НАЗАЧЕННЫЕ'
    else:
        qs = get_filtered_tasklist(request.user)
    cd=CurDate() #; fi=FilterIndicator()
    c.update(curdate=cd ,status_name=status_name)
